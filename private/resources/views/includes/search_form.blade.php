<form class="form-inline" method="get" action="tutors">
    ...
    <div class="form-group ">
        <select class="form-control" name="disciplines[]" id="disciplines" onchange="fetch_select(this.value);">
            ...
            <optgroup style="right: 0%; top: 0%;" label="<?php echo $discipline->name;?>" data-max-options="1" discipline_id="{{$discipline->id}}">
        ...
        <select class="form-control" name="level[]" id="level" multiple="">
            ...
            <optgroup label="{{$level->level}}" data-max-options="1">
                ...
                <option value="{{$levelChild->id}}"
                    {{ !empty(\Input::get('level')) ? in_array($levelChild->id , \Input::get('level'))   ? 'selected="selected"' : '' : ''}}>
    ...
    <div class="form-group ">
        <select class="form-control" name="location[]" id="location" multiple="">
            ...
            <optgroup label="{{$country->name}}" data-max-options="1">
                ...
                <option value="{{$categorieChild->id}}"
                    {{ !empty(\Input::get('location')) ? in_array($categorieChild->id , \Input::get('location'))   ? 'selected="selected"' : '' : ''}}>
                    {{$categorieChild->name}}</option>
                @endforeach
                @endif
                @endforeach
...
<script>
    $(document).ready(function () {
        $('#find').click(function (e) {
            if (!$('#location').val()) {
                $('#validation').text('Please Select Location of Tutor');
                $('#form_validation').modal('show');
                e.preventDefault();
            }
            if (!$('#sp #specialist').val()) {
                $('#validation').text('Please Select Specialism of Tutor');
                $('#form_validation').modal('show');
                e.preventDefault();
            }
            if (!$('#disciplines').val()) {
                $('#validation').text('Please Select Type of Tutor');
                $('#form_validation').modal('show');
                e.preventDefault();
            }
