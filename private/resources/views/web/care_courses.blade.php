	...
	<div class="container">
		<div class="row img_content_care" style=" margin-bottom: 50px; align-items: center;background: #f2f2f2;">
			<div class="col-md-6 text-center">
				<div class="inner-cotent">
					<p><strong>Care Providers, Childcare Providers, Hospices, NHS, Dr’s Surgeries,Hospitality and
							Retailers</strong></p>
					<strong style="color:#e8058e">Or any other related industries that require ½ or 1 day
						courses</strong>
				</div>
			</div>
			<div class="col-md-6">
			...
			<div class="col-md-4">
				<div class="cstm-ancor set-icon" id="accordion-{{$i}}">
					<p class="icon-cstm" id="heading-{{$i}}">
						<a href="#" class="collapsed collapsed-img-{{$i}}" data-toggle="collapse" data-target="#demo-{{$i}}" aria-expanded="false" aria-controls="demo-{{$i}}">
							<img src="web/images/care_icon-{{$i}}.png" />{{$categorieItem->name}}
						</a>
					</p>
					<div id="demo-{{$i}}" aria-labelledby="heading-{{$i}}" data-parent="#accordion-{{$i}}"
						aria-expanded="true" class="collapse" style="">
						<ul class="cstm-list">
							@foreach($categorieItem->children as $categorieChild)
							<li>
								<a href="{{url('course_description/'.encrypt($categorieChild->id))}}">{{$categorieChild->name}}</a>
							</li>
		...
		<div class="row img_content_care" style=" margin-bottom: 50px; align-items: center;background: #f2f2f2;">
			...
			<div class="col-md-6 text-center">
				<div class="inner-cotent">
					...
					<p><strong>If you require your staff to undergo ½ or full day, mandatory/regularity staff training
							then simply follow the steps below.</strong></p>
					<strong style="color:#e8058e; display:block; font-size: 18px; padding: 0 24px;">Please
						Note:</strong>
					<strong style="color:#e8058e; display:block; font-size: 18px; padding: 0 24px;">- All course
						certificates will be industry approved and in-house accredited.</strong>
					<strong style="color:#e8058e; display:block; font-size: 18px; padding: 0 24px;">- If you require
						accredited training, then please send an enquiry as there will be additional charges.</strong>
				</div>
			</div>
		</div>
		...
		<div class="row book_course_title multiple_course_section">
			<div class="col-md-12">
				...
				<h6>Select your courses from the below list. If no results are found, then you may need to book then
					individually, as single day courses.</h6>
			</div>
			...
			<form class="form-inline" method="get" action="tutors">
				...
				<div class="form-group " id="sp">
					...
					<select class="form-control" name="specialist[]" id="specialist" multiple="">
						@foreach($categories as $categorieItem)
						@if(isset($categorieItem->children['0']))
						<optgroup label="{{$categorieItem->name}}" data-max-options="1">
							@foreach($categorieItem->children as $categorieChild)
							<option value="{{$categorieChild->id}}"
								{{ !empty(\Input::get('specialist')) ? in_array($categorieChild->id , \Input::get('specialist'))   ? 'selected="selected"' : '' : ''}}>
				...
				<button type="submit" class="btn btn-primary serach_btn" id='find'><i class="fas fa-search"></i>Search
					Tutor</button>
			</form>
		</div>
	</div>
				...
				<nav>
					<div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
						<a class="nav-item nav-link active" id="nav-home-tab" data-toggle="tab" href="#nav-home"
							role="tab" aria-controls="nav-home" aria-selected="true">Single Day Course</a>
						<a class="nav-item nav-link" id="nav-profile-tab" data-toggle="tab" href="#nav-profile"
							role="tab" aria-controls="nav-profile" aria-selected="false">Multi Day Course</a>
					</div>
				</nav>
						...
						<div class="row">
							<div class="col-md-6 pull-left" data-aos="fade-right">
								<div class="inner_con_how_work green_color">
									...
									<div class="desc_con">
										<h3>Step</h3>
										<p>Choose a date and complete the form adding as much detail as possible and
											then hit ‘Next’</p>
									</div>
								</div>
							</div>
						...
						<div class="row">
							<div class="col-md-6 pull-left" data-aos="fade-right">
								<div class="inner_con_how_work green_color">
									...
									<div class="desc_con">
										<h3>Step</h3>
										<p>Make your Payment & receive an email confirmation of your booking, then your
											done!</p>
									</div>
								</div>
							</div>
						...
						<div class="row">
							...
							<div class="col-md-6 pull-right" data-aos="fade-left">
								<div class="inner_con_how_work">
									...
									<div class="desc_con">
										<h3>Step</h3>
										<p>Select the courses from multi select course dropdown list and hit 'Search
											Tutor' button – You may be required to login</p>
									</div>
								</div>
							</div>
						</div>
