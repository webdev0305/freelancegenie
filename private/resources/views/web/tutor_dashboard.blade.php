...
<section class="inner-cotent">
    <div class="container">
        ...
        <a target="_blank" href="tutor/tutor_swap" class="btn btn-info btn-lg">Swap Requests</a>
        ...
        <table id="example" class="table table-striped table-bordered table-responsive-lg" style="width:100%">
            ...
            <?php $date = explode(',', $job['userjobs']->date);
            ...
            //echo '<pre>';print_r($job);echo '</pre>';die('I am here');
            ...
            ?>
                ...
                <tr>
                    ...
                    <td>@php $status=$job['userjobs']->status;@endphp
                              @if($status == '1')                                {{'Booked'}}                                @endif                              @if($status == '0')                                {{'Awaiting Tutor Acceptance'}}                              @endif                              @if($status == '2')                                {{'Rejected'}}                              @endif                              @if($status == '3')                                {{'Completed'}}                              @endif                              @if($status == '4')                                {{'FTA'}}                              @endif                              @if($status == '5')                                {{'Cancelled'}}                              @endif                              @if($status == '6')                                {{'Booked'}}                              @endif                             @if($status == '7')                                {{'Declined by Employer'}}                              @endif                           @if($status == '8')                                {{'Awaiting Employer Approval'}}                              @endif
                    </td>
                    
                    <td>{{-- $status --}}
                        
                        @if($current_date <= $date_from && $status == "0")
                        <button type="button" class="announce btn btn-success float-left mr-1" data-id="{{encrypt($job['userjobs']->id)}}"  data-status="1"  data-toggle="modal" data-target="#deleteMerchant">Accept
                        </button>
                        @endif
                        @if($status == "0")
                        <button type="button" class="announce btn btn-danger float-left mr-1" data-id="{{encrypt($job['userjobs']->id)}}"  data-status="2"  data-toggle="modal" data-target="#deleteMerchant">Reject
                        </button>
                        @endif    
                            
                        @if(($status == "6" || $status == "1") && $current_date >= $date_from && $current_date <= $date_to)
                            <button type="button" class="update_record_btn btn btn-success float-left mr-1" data-id="{{$job['userjobs']->id}}"  data-status="2"  data-toggle="modal" data-target="#update_record">Confirm Arrival
                            </button>
                            <button type="button" class="update_register_btn btn btn-success float-left mr-1" data-id="{{$job['userjobs']->id}}"  data-status="2"  data-toggle="modal" data-target="#update_register">Update Register
                            </button>
                            @endif

                              @if(($status == "6" || $status == "1") && $current_date >= $date_to)
                            <button type="button" class="announce btn btn-success float-left mr-1" data-id="{{$job['userjobs']->id}}"  data-status="3"  data-toggle="modal" data-target="#deleteMerchant">Job Done
                            </button>
                            @endif
                    
                            @if($current_date >= $date_from && $status!=0)
                            
                            <a class="announce btn btn-success float-left mr-1" href="{{url('tutor/invoice').'/'.$job['userjobs']->id}}">View Invoice</a>
                            @endif
                            
                            @if(($current_day == "Mon" && $status !=0) || $status == '4')
                            <button type="button" class="invoice_sent btn btn-success float-left mr-1" data-id="{{$job['userjobs']->id}}"  data-tutor_id="{{$job['userjobs']->tutor_id}}">Send Invoice</button>
                            @endif
                            
                            @if((($status == '6' && $job['userjobs']->assignment) || (!$job['userjobs']->assignment && $status==1)) && ($current_date < strtotime('-3 Day',$date_from)))
...
<div class="modal" id="date_modal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">        <div class="modal-dialog" role="document">            <div class="modal-content manage_bookings_popup">                <div class="modal-header">                    <h5 class="modal-title" id="exampleModalLabel">Manage Bookings</h5>                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">                        <span aria-hidden="true">&times;</span>                    </button>                </div>                                <div class="modal-body">                    <div class="text-wrap">                        <p>You can see all your bookings here.</p>                    </div>                    <div class="row" id='md'> <!-- Calendar will be loaded here-->                    </div>                </div>                               <div class="modal-footer">                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>                </div>            </div>        </div>    </div>
<div class="modal" id="availability_mod" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <div class="row">
                    <div class="col-md-12 col-sm-12">
                        <div class="form-group">        
                            <div class="radio">
                                <label>
                                    <input type="radio" {{ (!$user->availability) ? 'checked' :''}} name="available" value="0">Unavailable
                                </label> 
                                <label>
                                    <input {{($user->availability) ? 'checked' :''}} type="radio" name="available" value="1">Available
                                </label>
                            </div>
                        </div>
                    </div>
...
<div class="modal maiilModal" id="dbs_expire" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
